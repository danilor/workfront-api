...
describe('E2E Tests', function () {
    ...
    describe('Login with username and password', function () {
        ...
        it('Creates a group "Api Group", edits the name to read "Api Group 2", then deletes it', function () {
            return this.instance.create('group', {name: 'Api Group'}).then((data) => {
                return this.instance
                    ...
                    .then((data) => this.instance.remove('group', data.ID))
            })
        })
        ...
        it('Creates a new project with name "API Project"', function () {
            ...
            const objCode = 'PROJ'
            return this.instance
                .create('proj', {name, description}, ['description'])
        ...
        it('Creates a new project with name "API Project Shared" and then shares it', function () {
            ...
            return this.instance
                ...
                .then((data) => {
                    ...
                    return this.instance
                        .create(objCode, {name, description}, ['accessRules:*'])
                        .then(putObjToRemoveQueue)
                        .then((data) => {
                            ...
                            return this.instance
                                .edit(objCode, data.ID, {accessRules: accessRules})
                                .then((data) => {
                                    should(data).be.Object()
        ...
        it('Creates a new project with name "API Project", then copies it without any option', function () {
            ...
            const objCode = 'PROJ'
            return this.instance
                .create(objCode, {name, description})
                .then(putObjToRemoveQueue)
                .then((data) => {
                    const originalProjectID = data.ID
                    return this.instance
                        .copy(objCode, originalProjectID, {name: copyName})
                        .then(putObjToRemoveQueue)
                        .then((data) => {
        ...
        it('Copies a project with options', function () {
            ...
            const description = 'This project has been created using API'
            const objCode = 'PROJ'
            const copyOptions = ['clearApprovers', 'clearProgress']
            return this.instance
                .create(objCode, {name, description})
                .then(putObjToRemoveQueue)
                .then((data) => {
                    const originalProjectID = data.ID
                    return this.instance
                        .copy(objCode, originalProjectID, {name: copyName}, null, copyOptions)
                        .then(putObjToRemoveQueue)
                        .then((data) => {
                            should(data).have.property('ID').not.eql(originalProjectID)
                        })
        ...
        it('Queries details of multiple tasks', function () {
            return this.instance
                ...
                .then((data) => {
                    ...
                    return Promise.all([
                        this.instance
                            .create('TASK', {name: 'foo', projectID})
                            .then(putObjToRemoveQueue),
                        this.instance
                            .create('TASK', {name: 'bar', projectID})
                            .then(putObjToRemoveQueue),
                        ...
                        return this.instance
                            .get('TASK', [taskFoo.ID, taskBar.ID], ['*'])
                            .then((data) => {
                                should(data).be.Array()
                                should(data).have.length(2)
                                should(data[0]).have.properties({
                                    ID: taskFoo.ID,
                                    name: taskFoo.name,
                                })
                                should(data[1]).have.properties({
                                    ID: taskBar.ID,
                                    name: taskBar.name,
                                })
                            })
